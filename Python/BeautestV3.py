...
def read_images (path , sz= (400,500)):
    c = 0
    X,y = [], []
...
while testFinServeur == False:
    try:
        ...
        Photo=connexion.recv(800000)
        Photo.decode("utf-8")
        R=Photo["I"]
        nb=int(R)
        photorecu=""
        a=0
        while (len(photorecu)<=nb):
            aqui=b"ok"
            aqui=aqui.encode()    
            print(a)
            a+=1
...
#
#
#        imtest = Image.fromarray(np.uint8(cm.gist_earth(pix)*255))
#    
#    
...
#    model = EigenfacesModel (X , y)
#    
#    print(" expected = ", expected_class[i], " / predicted =", L[model.predict(test)])
#    
#
#
#cv2.destroyAllWindows()
